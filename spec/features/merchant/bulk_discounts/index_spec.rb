...
describe "merchant bulk discounts index" do
  ...
  it "displays all bulk discounts including and info, and links to show page" do
    expect(page).to have_content(@bulk1.threshold)
    expect(page).to have_content(@bulk1.percent_discount)
    expect(page).to_not have_content(@bulk3.percent_discount)

    expect(page).to have_link "Bulk Discount ##{@bulk1.id}"
    expect(page).to have_link "Bulk Discount ##{@bulk2.id}"
    expect(page).to_not have_link "Bulk Discount ##{@bulk3.id}"
  end
  ...
  it "has a link to delete the bulk discount" do
    within ("#discount-#{@bulk1.id}") do
      ...
      expect(page).to_not have_link "Bulk Discount ##{@bulk1.id}"
  end
end
