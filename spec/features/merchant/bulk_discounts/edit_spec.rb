...
describe "merchant bulk discount edit" do
  ...
  it "can edit a bulk discount" do
    expect(find_field('Threshold').value).to eq("#{@bulk1.threshold}")
    expect(find_field('Percent discount').value).to eq("#{@bulk1.percent_discount}")

    expect(find_field('Threshold').value).to_not eq("#{@bulk2.threshold}")
  end
  ...
  it "can edit the bulk discount" do
    ...
    expect(page).to_not have_content("Item Threshold: 3")
    expect(page).to have_content("Percent Discount: 50")
    expect(page).to_not have_content("Percent Discount: 25")
