...
RSpec.describe 'invoices show' do
  before :each do
    ...
    @invoice_item_2 = InvoiceItem.create!(invoice_id: @invoice_2.id, item_id: @item_1.id, quantity: 1, unit_price: 10, status: 2)
    ...
    @invoice_item_10 = InvoiceItem.create!(invoice_id: @invoice_8.id, item_id: @item_5.id, quantity: 1, unit_price: 200, status: 1)
    @invoice_item_11 = InvoiceItem.create!(invoice_id: @invoice_1.id, item_id: @item_8.id, quantity: 12, unit_price: 5, status: 1)
  ...
  it "inclues bulk discounts in the calculation of total revenue" do
    ...
    @invoice_item_12 = InvoiceItem.create!(invoice_id: @invoice_1.id, item_id: @item_8.id, quantity: 25, unit_price: 6, status: 1)

    visit merchant_invoice_path(@merchant1, @invoice_1)
    expect(page).to have_content(@invoice_1.total_revenue_with_discounts)
  end

  it "shows a link for the discounts that were applied" do
    @bulk2 = @merchant1.bulk_discounts.create!(threshold: 5, percent_discount: 25)

    visit merchant_invoice_path(@merchant1, @invoice_1)
    within ("#the-status-#{@invoice_item_1.id}") do
      click_link "#{@bulk2.percent_discount}"
      expect(current_path).to eq(merchant_bulk_discount_path(@merchant1, @bulk2))
    end
  end
